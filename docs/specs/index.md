...
|15 | [`booking-insights.md`](booking-insights.md) | in progress — PR1 (lead-time histogram + Plus-tier `/reports/insights` surface + CSV export + `bookings_venue_created_idx`) + PR2 (no-show evolution line chart with client-side day/week/month/year rollup + per-channel performance table) landed; week/month/year period comparison in PR3. Extends `reporting.md`. |
