...
|13 | [`reviews.md`](reviews.md)   | shipped | Internal review capture (HMAC token URL + encrypted comment), operator dashboard + reply, Google OAuth + location picker + review pull/reply, escalation alerts + recovery offers, opt-in public showcase. Phase 4 (TripAdvisor / Facebook) + Phase 5 (AI sentiment) deferred; Phase 7b cut. |
...
|12 | [`ai-enquiry.md`](ai-enquiry.md)     | shipped — inbound webhook + Bedrock parser + runner/cron + operator inbox + 90-day retention sweep. Auto-send mode + per-venue verified sending domain deferred. |
