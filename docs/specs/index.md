...
|14 | [`venue-slug.md`](venue-slug.md) | shipped | Friendly public URLs (`book.tablekitapp.com/jane-cafe`); UUID URLs keep working + 308-redirect |
...
|14 | [`public-api.md`](public-api.md)     | shipped — Bearer-auth REST API at `api.tablekitapp.com/v1` (bookings + read-extras + idempotency) + webhook subscriptions/deliveries/replay + 90d request log + OpenAPI 3.1 + Stoplight docs at `/docs/api`. |
...
|24 | [`email-broadcast-billing.md`](email-broadcast-billing.md) | in review (PR; flag off — display-only until `EMAIL_OVERAGE_ENFORCED` flips at GA, which also gates marketing-site sync) — email broadcasts open to Core; per-plan monthly allowance (Core 500 / Plus 2,500) + PAYG overage from the existing prepaid credit ledger (Core £1.00 / Plus £0.90 per 1,000 ex-VAT — 40%/33% margin over Resend's $0.80/1,000, under Brevo, ~6–7× under Mailchimp; VAT already collected at top-up via Stripe Tax). Reserve/reconcile extended with allowance+rate snapshots (same-function invariant kept); consumption derived from `campaign_sends`; SMS/WhatsApp broadcasts + segments stay Plus; transactional email stays free/unblocked. |
|25 | [`marketing-suite.md`](marketing-suite.md) | in review (PR; Phases A+B+C incl. templates + starters — overview page, link-level clicks + marketing-site sync remain for GA) — block-based email builder (image/button/divider/spacer, then booking-CTA/countdown/event-card + saved templates) rendered to email-safe HTML, and campaign reporting with **booking/covers attribution** (`tk_c` link param + 7-day click-window fallback, `bookings.campaign_id`). Builder + basic stats Core+; attribution funnel + overview dashboard Plus. Opens flagged as MPP-inflated; clicks + bookings lead. Phases A/B/C. |
...
|28 | [`area-preferences.md`](area-preferences.md) | in progress — guest **area preference** on the widget Time step (chips; preference = guarantee, enforced through `createBooking.preferredAreaId`) + operator **area availability** (`areas.bookable` weather kill-switch + `areas.closed_months` seasonal closures). Both compile into the special-events `ClosureWindow{areaIds}` machinery — one engine path, no new availability concepts. Ungated, all tiers. |
