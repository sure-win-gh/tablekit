...
|12a| [`table-combining.md`](table-combining.md) | draft | Operator-controlled table joins: `table_combinations` adjacency-graph table (RLS, same-area denorm trigger) + visual "Set up table joins" mode on the floor plan (lines hidden outside setup) + connected-free-set enumeration in the availability engine (`maxTables` cap, per-area declared-edges-only, legacy same-area pairs as the no-config fallback). Extends `bookings.md` + `floor-plan-visual.md`. |
