...
|19 | [`marketing-campaigns.md`](marketing-campaigns.md) | draft — Plus-tier broadcast: `campaigns` + `campaign_sends` + `message_usage` tables (RLS), consent-gated guest-scoped queue + dispatch worker, composer with audience/cost estimate + preview, Resend open/click engagement tracking, pass-through usage metering. `/api/cron/campaign-tick`. |
