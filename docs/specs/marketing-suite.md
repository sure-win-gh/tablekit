...
**Marketing overview (2026-07-10, built):** `/dashboard/venues/[venueId]/campaigns/reports` — trailing-90-day roll-up across all campaigns (`lib/campaigns/overview.ts`): performance **by channel** (campaigns/sends/delivered/opened/clicked/bookings/covers), **top campaigns by booking conversion** (bookings ÷ delivered, `max(delivered,sent)` base mirroring the per-campaign report), and **audience health** (per-channel opted-in list size, new opt-ins in the window, current unsubscribe share — consent predicate matches `audiencePredicate` exactly, so "opted-in list" = who a send would actually reach). Rates carry the Apple-MPP opens caveat. **Plus-gated server-side** (the headline upsell — Core sees the upsell card, not the numbers); attribution capture still runs for all tiers so the funnel back-fills on upgrade. CSV export route (`reports/export`, audit-logged `data.exported`, three sections in one file via a `section` column). Read via `withUser` → RLS org-scopes everything; audience health is deliberately org-scoped (consent lives on the guest at org level, opt-out is per-venue). Cross-tenant isolation + aggregate correctness proven in `tests/integration/rls-marketing-overview.test.ts`. Deviation: unsubscribe rate is a current snapshot (no per-opt-out timestamp), labelled as such; new-opt-ins is gross adds in the window.

**Link-level clicks (2026-07-11, built):** `campaign_link_clicks` table (migration 0061 — one row per `(campaign_send, url)`; the unique index dedupes repeat clicks so the report counts **unique clickers per URL**, not raw volume). `organisation_id` populated by `enforce_campaign_link_clicks_org_id` from the parent campaign; member-read RLS, adminDb-only writes. Fed by the Resend `email.clicked` webhook (`event.data.click.link` → `onConflictDoNothing` on `(send, url)`) — only the URL is stored, never the IP / user-agent Resend also sends. Surfaced as a "Most-clicked links" section on the per-campaign report (Core+ engagement detail, email only). `campaign_send_id` FK cascades on delete, so DSAR erasure (`lib/dsar/scrub.ts` deletes the guest's sends) and the 24-month retention sweep both remove link clicks with **no extra scrub path** — proven by the cascade test in `tests/integration/rls-campaign-link-clicks.test.ts` (also covers the enforce trigger, unique dedup, aggregation, and cross-tenant RLS isolation).

**Phase C complete.** All rich blocks, templates, overview dashboard, and link-level clicks shipped.
