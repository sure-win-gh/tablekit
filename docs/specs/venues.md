...
- [x] Tables combinable at booking time when `max_cover` is exceeded. [`lib/bookings/availability.ts`](../../lib/bookings/availability.ts) — a venue with no operator-set joins combines any two tables that share an area; once an operator draws joins for an area, only those declared combinations are offered (a connected set of free tables). See [`table-combining.md`](table-combining.md). Unit-tested in `tests/unit/bookings-availability.test.ts`.
