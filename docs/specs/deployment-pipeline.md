...
- **Fix (this PR):** snapshot the edge's `commit` (or `<none>`) as `PRE_COMMIT` immediately before the promote, then after the primary flip poll `/api/health` (cap 180s) until the target sha appears **or** the served value leaves `PRE_COMMIT` (absence included). Deadline splits two ways: edge unreachable or the departure unobservable (a `<none>`→`<none>` pre-#136→pre-#136 swap) → `::warning::`, green, because the primary already confirmed; edge reachable and still definitely serving a concrete `PRE_COMMIT` after 180s → red **EDGE STALE**, stating the promote succeeded at the Vercel layer, not to re-run, and to check the Cloudflare cache and DNS for `my.tablekitapp.com`. Primary assertion, id-based guard, trigger-timestamp first line, and trigger→flip elapsed are unchanged.
