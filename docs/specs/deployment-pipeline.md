...
> **Gotcha — a `$` in the DB password looks like a dead database.** Supabase auto-generates pooler passwords that often contain `$` (e.g. `QN4Gpv$8C3P8NGS`). In a double-quoted or unquoted shell assignment, the shell expands `$8` as positional parameter 8 (empty), silently mangling the password — `db:migrate` then hangs on its spinner and exits with no useful error, which reads as an unreachable or broken project. It isn't: the URL is corrupted before Postgres ever sees it. Provisioning `tablekit-staging` (2026-07-29) lost time to exactly this; the project was healthy (PG 17.6, all migrations + RLS + extensions clean) the moment the URL was quoted correctly. When running a migration by hand, **single-quote** the whole `DATABASE_URL` (`DATABASE_URL='postgresql://…$8…' pnpm db:migrate`) or percent-encode the `$` as `%24`. GitHub Actions secrets are a literal env injection with no shell expansion, so CI and the promote/staging workflows are unaffected — this bites only manual/local invocations.
...
Residual caveats to document in `incident.md`: webhooks keep arriving during the swap (Stripe retries for 3 days — already noted there); Vercel Cron *invocations* hit whatever production is aliased to, so the code a job runs does roll back with the alias (desired) — but the *schedules themselves do not*, per the next paragraph.

**Instant Rollback does not update active cron jobs.** From Vercel's [cron-jobs docs](https://vercel.com/docs/cron-jobs/manage-cron-jobs#rollbacks-with-cron-jobs): "If you Instant Rollback to a previous deployment, active cron jobs **will not** be updated. They will continue to run as scheduled until they are manually disabled or updated." So after a rollback the registered job list and schedules are still those of the build you rolled back **from**, while each invocation executes the code of the build you rolled back **to**. With 17 jobs in `vercel.json` this is not hypothetical — if the bad release added, retimed, or removed a cron entry, rolling back does **not** undo that change. Two failure modes to check whenever a rollback crosses a `vercel.json` cron change:

- A job the bad release **added** keeps firing on schedule against code that may not have the route. Per [Vercel's docs on nonexistent paths](https://vercel.com/docs/cron-jobs/manage-cron-jobs#handling-nonexistent-paths) — "If you create a cron job for a path that doesn't exist, it generates a 404 error. However, **Vercel still executes your cron job**" — so this surfaces as recurring 404s in the logs rather than silence.
- A job the bad release **retimed or removed** keeps the new timing (or keeps running) until you act.

Rolling back again does not fix either — the schedule registry only changes on deploy. Correct it by editing `vercel.json` and redeploying, or by disabling the job in Settings → Cron Jobs. Verify against the dashboard's cron list, **not** against `vercel.json` in the rolled-back working tree, which will disagree with what is actually scheduled.
