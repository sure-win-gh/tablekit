...
- **Booking a table / buying an event ticket:** contract (Art 6(1)(b)). We don't need consent to store a booking or ticket purchase the guest is creating. An abandoned ticket checkout (guest row created before payment; the janitor cancels the booking after 15 minutes) rests on the same basis — the guest row it leaves behind follows the standard "until guest or org erases" retention rule.
...
| Event ticket purchases (`event_order_items`: quantity + price snapshot; `event_ticket_types`: operator config; `payments.kind='event_ticket'`) | 7 years (alongside booking history / payment metadata) | Order items carry no PII and survive as accounting data; the parent booking is pseudonymised via guest erasure like any booking |
...
| AI usage tallies (`ai_usage`: per-month per-org/venue Bedrock call + token counts) | Indefinite (billing record) | No — non-PII aggregate counts only, no guest linkage. Per-enquiry attribution rides the audit log (`enquiry.ai_parse`, non-PII metadata, 2-year audit retention). |
