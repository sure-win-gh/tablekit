...
| Marketing campaign sends + engagement (`campaign_sends`: status, opens/clicks, keyed to `guest_id`) | Until guest erasure | Yes — rows deleted outright on guest erasure (`lib/dsar/scrub.ts`). Behavioural data, no accounting retention. |
